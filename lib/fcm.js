...
function init() {
    ...
    try {
        ...
        // ═════════════════════════════════════════════════════════════
        // REUSE AN EXISTING APP. A SECOND initializeApp() THROWS.
        //
        // firestore.init() has always had this guard and this did not, which was
        // safe only because server.js happens to call fcm.init() first. That is
        // a fact about one line's position in one file, defended by nothing —
        // and the failure it permits is silent: the throw lands in the catch
        // below, fcmReady stays false, and FCM is disabled for the process. The
        // symptom is "the dashboard toggle takes 30 seconds", which is the
        // poller doing FCM's job, and it looks nothing like an init error.
        //
        // Both initialisers now reuse whatever is there, so neither depends on
        // running first.
        // ═════════════════════════════════════════════════════════════
 ...
 */
const TTL_MS = {
    // An override is time-boxed and the server enforces the end, so a late
    // delivery is pointless — same reasoning as the one-shot commands.
    override_on:  2 * 60 * 1000,
    override_off: 2 * 60 * 1000,
    // A server move must reach a phone that has been asleep or off data. It is
    // also idempotent — being told the URL you already have is a no-op — so a
    // long life costs nothing and a short one strands a phone on a dead server.
    set_url:   60 * 60 * 1000,
    enable:    15 * 60 * 1000,
    disable:   15 * 60 * 1000,
    test:       2 * 60 * 1000,
 ...
 */
async function send(action, ts, topic, extra) {
    if (!fcmReady || !admin) {
    ...
    try {
        ...
        const result = await admin.messaging().send({
            ...
            data: Object.assign({
                action:    action,
                timestamp: timestamp
            }, extra || {}),   // set_url carries the new URL here
            android: {
                priority: 'high',
                ttl: ttl
            }
