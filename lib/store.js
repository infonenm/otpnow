 ...
 */
function waitForOtp(number, timeoutMs, onCancel) {
    ...
    return new Promise((resolve) => {
        ...
        const entry = {
            wake: () => {
                ...
                // going through getOtp keeps expiry and supersede in one place.
                const otp = getOtp(normNumber);
                if (otp) finish(otp);
            }
        };
...
function getOtp(number) {
    ...
    const candidates = messagesFor(normNumber).filter(sms => sms.status === 'pending');
...
// ─── Exports ────────────────────────────────────────────────────
module.exports = {
    addSms, getOtp, getAllSms, clearAll, waitForOtp, unconsume,
