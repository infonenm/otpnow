...
// Commands older than this are stale and won't be sent to devices.
// Prevents test/clear_log from re-firing on every app restart.
const COMMAND_MAX_AGE_MS = 120_000;  // 2 minutes

function getSettings() {
    const now = Date.now();
    return {
        globalForwarding:  settings.globalForwarding,
        clearLogTs:        (settings.clearLogTs > 0 && now - settings.clearLogTs < COMMAND_MAX_AGE_MS) ? settings.clearLogTs : 0,
        testMessageTs:     (settings.testMessageTs > 0 && now - settings.testMessageTs < COMMAND_MAX_AGE_MS) ? settings.testMessageTs : 0,
