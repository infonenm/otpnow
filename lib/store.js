...
function addSms(sender, recipient, message, arrivedAt) {
    ...
    // Update /number/ fast-lookup
    if (code) {
        const existing = numberMap.get(recipient);
        if (!existing || !existing.ts || existing.ts <= now) {
            numberMap.set(recipient, { otp: code, smsKey: id, ts: now });
        }
    }
...
function getOtp(number) {
    ...
    // Fallback: scan smsMap for this recipient
    const candidates = [];
    ...
    for (const sms of candidates) {
        const age = Date.now() - sms.arrivedAt;
        ...
        sms.status   = 'used';
        sms.viewedAt = Date.now();
        numberMap.set(number, { otp: code, smsKey: sms.id, ts: sms.arrivedAt });
        broadcast('sms_update', sms);
        return String(code);
    }
