...
function getOtp(number, senderTokens) {
    ...
    const candidates = messagesFor(normNumber)
        // `stale` excluded for the same reason it may not supersede: a code that
        // was already too old when it arrived must never be served as current.
        .filter(sms => sms.status === 'pending' && !sms.stale
                    && senderMatches(sms.sender, senderTokens));
