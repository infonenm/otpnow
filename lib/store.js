...
function loadIdentity() {
    ...
    firestore.readIdentity().then(data => {
        const n = users.load(data || {});
        console.log(`[store] Identity loaded: ${n.users} user(s), ${n.devices} device(s)`);
    }).catch(e => {
        console.error(`[store] Identity load failed (${e.message}) — starting with none`);
    });
}
...
function loadDurableConfig() {
    ...
    firestore.readConfig().then(cfg => {
        clearTimeout(ceiling);
        if (cfg) {
            applyConfig(cfg, 'Firestore');
            return;
        }
        // No document yet. Push what we have up, so the next redeploy has
        // something to restore from. Migration runs once, by definition.
        markConfigReady();
...
function addSms(sender, recipient, message, arrivedAt, deviceId) {
    ...
    // ─────────────────────────────────────────────────────────────
    if (normRecip === 'Unknown' && deviceId) normRecip = 'Unknown-' + deviceId;
...
module.exports = {
    ...
    forwardingFor, overrideCeilingMinutes, commandsFor,
