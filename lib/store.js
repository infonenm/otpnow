...
function validateToken(token) {
    ...
    if (Date.now() - created > TOKEN_TTL) { tokens.delete(token); return false; }
    return true;
}
...
function addSms(sender, recipient, message, arrivedAt) {
    ...
    const now       = arrivedAt || Date.now();
    const code      = extractCode(message, sender, recipient, settings.filters);
    const deleteAt  = now + settings.autoDeleteMinutes * 60_000;
    ...
    if (code) {
        ...
        // Old OTP is dead the moment a new SMS arrives — matches old system
        for (const [oldId, oldSms] of smsMap) {
            if (oldId !== id && oldSms.recipient === recipient && oldSms.status === 'pending') {
                oldSms.status = 'superseded';
                broadcast('sms_update', oldSms);
            }
        }

        // Replace the fast-lookup entry (fresh OTP, not yet consumed)
        numberMap.set(recipient, { otp: code, smsKey: id, ts: now, consumed: false });
    }

    broadcast('sms_new', sms);
    console.log(`[store] ${id} from=${sender} to=${recipient} code=${code || '(none)'}`);
    return { id, code };
}
...
function getOtp(number) {
    ...
    if (entry && entry.otp) {
        ...
        // Entry stays in map so we know it was already read
        // (will be replaced when new SMS arrives via addSms)
        entry.consumed = true;
    ...
    for (const sms of smsMap.values()) {
        if (sms.recipient === number && sms.status === 'pending') {
            candidates.push(sms);
        }
    }
    ...
    for (const sms of candidates) {
        ...
        sms.status   = 'used';
        sms.viewedAt = Date.now();
        numberMap.set(number, { otp: code, smsKey: sms.id, ts: sms.arrivedAt, consumed: true });
        broadcast('sms_update', sms);
        return String(code);
    }
