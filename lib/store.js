...
function persistState() {
    ...
    try {
        ...
        // Durable copy, debounced and NOT awaited. The state file is the warm
        // local cache; Firestore is what survives a redeploy. The dashboard's
        // response never waits on Google.
...
function loadIdentity() {
    ...
    users.setPersister(snap => firestore.writeIdentity(snap));
    ...
    firestore.readIdentity().then(result => {
        if (!result.ok) {
            // DO NOT load, and do not let anything be written. users.load()
            // clears the maps, and the next automatic change would push that
            // empty set to Firestore — deleting every account permanently.
            console.error('[store] Identity unreadable — user accounts are NOT loaded and '
                + 'NOTHING will be written until a read succeeds. Existing accounts in '
                + 'Firestore are untouched. Devices file to admin meanwhile.');
            return;
        }
        const n = users.load(result.data || {});
        console.log(`[store] Identity loaded: ${n.users} user(s), ${n.devices} device(s)`);
    }).catch(e => {
        console.error(`[store] Identity load failed (${e.message}) — NOT writing anything`);
    });
}

function loadDurableConfig() {
    if (!firestore.isEnabled()) { markConfigReady(); return; }
    ...
    firestore.readConfig().then(result => {
        ...
        // ─────────────────────────────────────────────────────────
        if (!result.ok) {
            console.error('[store] Firestore unreadable — keeping local config and NOT '
                + 'migrating. Your saved config in Firestore is untouched.');
            return;
        }
        if (!hasPersistedConfig()) {
        ...
        clearTimeout(ceiling);
        console.error(`[store] Durable config load failed (${e.message}) — using ${configSource}`);
        markConfigReady();
    });
}
...
function startOverride(userId, minutes) {
    ...
    broadcast('settings_change', { key: 'override', value: { userId, expiresAt } });
...
function cancelOverride(userId) {
    ...
    if (had) {
        broadcast('settings_change', { key: 'override', value: { userId, expiresAt: 0 } });
