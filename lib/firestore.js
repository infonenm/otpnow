...
let configInFlightPromise = null;

async function flushWrite() {
    if (!enabled || !pendingValue) return false;
    // A failure schedules its own retry, and a save during the outage arms the
    // debounce timer too — so two chains could run against the same document at
    // once, sharing one counter, which SHORTENS the backoff under load instead
    // of lengthening it. One in-flight write at a time.
    if (configInFlight) return configInFlightPromise;
    configInFlight = true;
    configInFlightPromise = doFlushWrite()
        .finally(() => { configInFlight = false; configInFlightPromise = null; });
    return configInFlightPromise;
}

async function doFlushWrite() {
    if (!pendingValue) return false;
    // NOT cleared here. Only a successful write may drop it — and only if it is
    // still THIS value; see the success branch.
    const mine = pendingValue;
    const { config, updatedBy } = mine;
    ...
    try {
        ...
        // ═════════════════════════════════════════════════════════
        // ONLY IF NOBODY SAVED AGAIN WHILE THIS WAS IN FLIGHT.
        //
        // This cleared the slot unconditionally. A cold Firestore connection
        // routinely takes seconds, and a second save landing inside that window
        // was overwritten to null — never written — while pendingConfig flipped
        // to false, so the dashboard reported "saved".
        //
        // Reproduced: save A, then save B while A is in flight ->
        //   stored filters: [{"phoneNumber":"A"}]   pendingWrite: false
        //
        // Same class as the "Retry now erased a user" bug, through the
        // IN-FLIGHT window instead of the debounce window.
        // ═════════════════════════════════════════════════════════
        if (pendingValue === mine) {
            pendingValue = null;
            status.pendingConfig = false;
        } else {
            console.log('[firestore] A newer config arrived mid-write — flushing it now');
            const t = setTimeout(flushWrite, 0);
...
/**
 * Race a Firestore call against a deadline, and CLEAR THE TIMER when it wins.
 *
 * Promise.race settles, but the loser keeps running: every call left a 20s
 * timer (40s for history) alive on the event loop. With the history flush every
 * 30 seconds there was always one or two pending — harmless in effect, and
 * exactly the kind of thing that makes a process refuse to exit and a test
 * suite hang for no visible reason.
 */
function withTimeout(promise, ms, label) {
    let timer = null;
    const deadline = new Promise((_, reject) => {
        timer = setTimeout(
            () => reject(new Error(`Firestore ${label} timed out after ${ms}ms`)), ms);
...
let identityInFlightPromise = null;

async function flushIdentity() {
    if (!enabled || !identityPending) return false;
    if (identityInFlight) return identityInFlightPromise;   // see flushWrite
    identityInFlight = true;
    identityInFlightPromise = doFlushIdentity()
        .finally(() => { identityInFlight = false; identityInFlightPromise = null; });
    return identityInFlightPromise;
}

async function doFlushIdentity() {
    if (!identityPending) return false;
    const data = identityPending;      // kept until the write succeeds
    const mine = data;                 // compared by identity on success
    try {
        ...
        // See doFlushWrite: a user created while this write was in flight was
        // otherwise discarded, with the dashboard reporting success.
        if (identityPending === mine) {
            identityPending = null;
            status.pendingIdentity = false;
        } else {
            console.log('[firestore] A newer identity arrived mid-write — flushing it now');
            const t = setTimeout(flushIdentity, 0);
            if (typeof t.unref === 'function') t.unref();
        }
...
/**
 * Push anything still pending, and wait for it.
 *
 * =============================================================================
 * A RELOAD MUST NOT RACE THE WRITE IT IS ABOUT TO OVERWRITE
 *
 * Writes are debounced by two seconds. Re-reading inside that window returns the
 * document as it was BEFORE the pending change, and users.load() then replaces
 * memory with it — so a user created moments earlier simply disappears. Found by
 * probing the endpoints in order: every request from that user's session
 * answered 401 afterwards, because the account it belonged to no longer existed.
 *
 * Worse, loadedOk stays true through a reload, so the next ordinary change would
 * have written that emptier state back as fact.
 *
 * So: flush first, then read. Nothing about this is on the OTP path.
 * =============================================================================
 */
async function flushNow() {
    ...
    // AWAIT AN IN-FLIGHT WRITE rather than skipping it. flushWrite returns
    // false immediately when one is already running, so a reload could return
    // having flushed nothing — the exact race the flush-first design exists to
    // close. Waiting for the in-flight promise closes it properly.
    try { await (configInFlightPromise   || flushWrite()); }    catch (e) { /* logged inside */ }
    try { await (identityInFlightPromise || flushIdentity()); } catch (e) { /* logged inside */ }
}
...
module.exports = {
    ...
    readIdentity, writeIdentity, flushNow,
