...
async function flushWrite() {
    ...
    configInFlight = true;
    try { return await doFlushWrite(); } finally { configInFlight = false; }
}
...
async function flushIdentity() {
    ...
    if (identityInFlight) return false;          // see flushWrite
    identityInFlight = true;
    try { return await doFlushIdentity(); } finally { identityInFlight = false; }
}
...
async function flushNow() {
    ...
    try { await flushWrite(); }    catch (e) { /* logged inside */ }
    try { await flushIdentity(); } catch (e) { /* logged inside */ }
}
