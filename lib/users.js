...
let allowedHosts = [];
let dirty = false;           // set by every mutation, cleared by the persister
...
function setPassword(id, enrollCode, password) {
    ...
    const given = String(enrollCode || '').trim().toUpperCase();
    if (given !== user.enrollCode) return { ok: false, error: 'Wrong enrollment code' };
...
function purgeUser(id) {
    ...
    for (const d of devices.values()) {
        if (d.userId === key) { d.userId = null; unassigned++; }
    }
    markDirty();
 ...
 * so reinstalling the app or restarting the phone does not orphan it.
 *
...
function registerDevice(deviceId, info) {
    ...
    const device = existing || {
        id, name: '', userId: null, model: '', createdAt: Date.now(), lastSeen: 0
    };
...
function ownerOf(deviceId) {
    const d = devices.get(String(deviceId || ''));
    if (!d || !d.userId) return ADMIN_ID;
    if (!isActive(d.userId)) return ADMIN_ID;      // deactivated owner: file to admin
    return d.userId;
}
...
function snapshot() {
    return {
        ...
        devices: Array.from(devices.values()).map(d => ({
            id: d.id, name: d.name, userId: d.userId, model: d.model,
...
module.exports = {
    ...
    getDevice, listDevices, ownerOf, deviceLabel,
