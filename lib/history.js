...
function record(sms, outcome) {
    ...
    if (queue.length >= MAX_QUEUE) {
        dropped++;
        // Throttled by the counter itself — this logs once per flush, not once
        // per message, or a Firestore outage would flood the log.
        return;
    }
    queue.push({
        smsId:       sms.id,
...
module.exports = {
    record, flush, start, stats, enabled, retentionDays,
